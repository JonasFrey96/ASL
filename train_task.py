...
def train_task(
  ...
  print(
    f"<<<<<<<<<<<< FINISHED TASK IDX {task_nr} TASK NAME : "
    + task_name
    + " Trained >>>>>>>>>>>>>"
  )
  ...
  if exp["replay"]["cfg_rssb"]["elements"] != 0:
    ...
    test_res = trainer.test(model=model, test_dataloaders=test_dataloader)
    ...
    print(
      "gtli [0]",
      train_dataloader.dataset.main_dataset.global_to_local_idx[0],
      "gtli [-1]",
      train_dataloader.dataset.main_dataset.global_to_local_idx[-1],
    )

    print(
      "Path [0]",
      train_dataloader.dataset.main_dataset.image_pths[
        train_dataloader.dataset.main_dataset.global_to_local_idx[0]
      ],
      "Path [-1]",
      train_dataloader.dataset.main_dataset.image_pths[
