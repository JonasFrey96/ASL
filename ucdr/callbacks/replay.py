...
class ReplayCallback(Callback):
  ...
  def on_train_start(self, trainer, pl_module):
    if pl_module._exp["replay"]["cfg_rssb"]["elements"] != 0:
      ls_global_indices = (
        trainer.train_dataloader.dataset.datasets.get_replay_datasets_globals()
      )
      ...
      trainer.train_dataloader.dataset.datasets.set_replay_datasets_globals(
        ls_global_indices
      )
  ...
  def on_train_end(self, trainer, pl_module):
    if pl_module._exp["replay"]["cfg_rssb"]["elements"] != 0:
      ls_global_indices = (
        trainer.train_dataloader.dataset.datasets.get_datasets_globals()
      )
      ...
      for bin, ls in enumerate(ls_global_indices):
        ...
        if el >= len(ls):
          ...
          pl_module._rssb.bins[bin, : len(ls)] = torch_ls[
            torch.arange(0, len(ls), dtype=torch.long)
          ]
