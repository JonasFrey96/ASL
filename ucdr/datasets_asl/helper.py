...
class Augmentation:
  def __init__(
    self, output_size=400, degrees=10, flip_p=0.5, jitter_bcsh=[0.3, 0.3, 0.3, 0.05]
  ):
  ...
  def apply(self, img, label, only_crop=False):
    ...
    if scale:
      ...
      label = torch.nn.functional.interpolate(
        label[None], scale_factor=(sf, sf), mode="nearest", recompute_scale_factor=False
      )[0]
    if not only_crop:
      # Color Jitter
      ...
      with warnings.catch_warnings():
        ...
        warnings.simplefilter("ignore")
        img = F.rotate(
          img, angle, resample=PIL.Image.BILINEAR, expand=False, center=None, fill=0
        )
        label = F.rotate(
          label, angle, resample=PIL.Image.NEAREST, expand=False, center=None, fill=0
        )
  ...
  def apply(self, img, label, only_crop=False):
    ...
    if not only_crop:
      ...
      with warnings.catch_warnings():
        ...
        for _i, l in enumerate(label):
          label[_i] = F.rotate(
            l, angle, resample=PIL.Image.NEAREST, expand=False, center=None, fill=0
          )
