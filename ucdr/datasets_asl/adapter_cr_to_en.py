...
def adapter_tg_to_en(tg, task_nr, replay_cfg_ensemble, env):
  ...
  for idx, task in enumerate(tg):
    if idx < task_nr + 1:
      ...
      if idx < task_nr:
        cfg_train_dataset["data_augmentation"] = replay_cfg_ensemble.get(
  ...
  train_dataset = Ensemble(
    main_dataset=train_dataset_list[-1], replay_datasets=replay_datasets, probs=probs
  )
...
def adapter_tg_to_dataloader(tg, task_nr, loader_cfg, replay_cfg_ensemble, env):
  train_dataset, val_datasets, task_name = adapter_tg_to_en(
    tg, task_nr, replay_cfg_ensemble, env
  )
...
def test():
  ...
  train, vals, task_name = adapter_tg_to_dataloader(
    tg, 0, exp["loader"], exp["replay"]["cfg"], env
  )
  print(tg)
  print(train)
  print(vals)
  ...
  train, vals, task_name = adapter_tg_to_dataloader(
    tg, 2, exp["loader"], exp["replay"]["cfg"], env
  )
  print(task_name)
