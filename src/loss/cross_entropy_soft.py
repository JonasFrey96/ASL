...
def test():
  ...
  label = torch.nn.functional.one_hot(label_ori.clone(), num_classes=C).permute(
    0, 3, 1, 2
  )
  ...
  res2 = torch.nn.functional.cross_entropy(
    pred, label_ori, ignore_index=-1, reduction="none"
  ).mean(dim=[1, 2])
