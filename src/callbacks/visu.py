...
class VisuCallback(Callback):
  ...
  def training_step_end(self, trainer, pl_module, outputs):
    # Logging + Visu
    if (self.visu_cfg['images'][pl_module._mode] > self.logged_images[pl_module._mode] and 
  ...
  def validation_step_end( self, trainer, pl_module, outputs ):
    ...
    if (self.visu_cfg['images']['val'] > self.logged_images['val'] and 
        pl_module.current_epoch % self.visu_cfg['every_n_epochs'] == 0) :
      self._visu (trainer, pl_module, outputs, pl_module._task_count, optional_key = 'Eval' + str( dataloader_idx) )
  ...
  def _visu( self, trainer, pl_module, outputs, task_nr, optional_key=""):
      ...
      if "aux_label" in outputs:
        ...
        grid_aux_pred = make_grid(aux_pred[:,None].repeat(1,3,1,1),nrow = rows,padding = 2,
              scale_each = False, pad_value = 0)
        self.visualizer.plot_segmentation(  tag=f'', seg=grid_aux_pred[0], method='right')
        self.visualizer.plot_segmentation(  tag=f'{pl_module._mode}_Task{task_nr}_{optional_key}_AUX_left_pred_right_{nr}', 
                                            seg=grid_aux_label[0], method='left')  
        self.visualizer.plot_segmentation(  tag=f'', seg=grid_label[0], method='right')
        self.visualizer.plot_segmentation(  tag=f'{pl_module._mode}_Task{task_nr}_{optional_key}__AUX_left_GT_right_{nr}', 
                                            seg=grid_aux_label[0], method='left')  
      self.visualizer.plot_segmentation(  tag=f'', seg=grid_pred[0], method='right')
      self.visualizer.plot_segmentation(  tag=f'{pl_module._mode}_Task{task_nr}_{optional_key}__gt_left_pred_right_{nr}', 
