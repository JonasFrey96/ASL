...
class CocoStuffBase(Dataset):
  def __init__(self, root='/home/jonfrey/Datasets/cocostuff', 
...
class CocoStuff164k(CocoStuffBase):
  ...
  def _load(self, root, mode):
    ...
    if mode in ["train", "test", "all"]:
      short = {"train": "train2017/*.jpg",
               "test": "val2017/*.jpg",
               "all": "*.jpg" }
      self.image_pths = sorted(glob( os.path.join( root, "images", short[mode]) ))
      self.label_pths = [ img_p.replace("/images/","/annotations/" ).replace(".jpg",".png") for img_p in self.image_pths]
...
def test():
  ...
  ds = CocoStuff164k( root='/home/jonfrey/Datasets/cocostuff', 
              mode='train', scenes=[], output_trafo = None, 
              output_size=400, degrees = 10, flip_p = 0.5, 
              jitter_bcsh=[0.3, 0.3, 0.3, 0.05], nyu40_labels = True)
  ...
  res = visu.plot_detectron( img = img_ori, label = label.numpy()+1, tag='test', jupyter=False, store=False,alpha=0.6)
