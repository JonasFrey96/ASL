...
class ScanNet(Dataset):
  def __init__(
    ...
    self._label_loader = LabelLoaderAuto(
      root_scannet=root, confidence=self._confidence_aux
    )
    if self.aux_labels:
  ...
  def _load(self, root, mode, train_val_split=0.2, label_setting="default"):
    ...
    self.train_test, self.scenes, self.image_pths, self.label_pths = self._load_cfg(
      root, train_val_split
    )
    self.image_pths = [
      os.path.join(root, i[1:]) for i in self.image_pths if i.find("scene0088_03") == -1
    ]
    self.label_pths = [
      os.path.join(root, i[1:]) for i in self.label_pths if i.find("scene0088_03") == -1
    ]

    if label_setting != "default":
      self.aux_label_pths = [
        i.replace("label-filt", label_setting) for i in self.label_pths
      ]
    ...
    if mode.find("_strict") != -1:
      ...
      colors = [
        str(p)
        for p in Path(r).rglob("*color/*.jpg")
        if str(p).find("scene0088_03") == -1
      ]
      fun = (
  ...
  def get_classes(train_val_split=0.2):
    data = pickle.load(
      open(f"cfg/dataset/scannet/scannet_trainval_{train_val_split}.pkl", "rb")
    )
  ...
  def _preprocessing_hack(self, force=False):
    ...
    if method == "RGBA":
      ...
      if self.aux_label_pths[self.global_to_local_idx[0]].find("_.png") == -1:
        ...
        if (
          os.path.isfile(
            self.aux_label_pths[self.global_to_local_idx[0]].replace(".png", "_.png")
          )
          and os.path.isfile(
            self.aux_label_pths[self.global_to_local_idx[-1]].replace(".png", "_.png")
          )
          and not force
        ):
          ...
          tasks = [
            t.tolist()
            for t in np.array_split(np.array(self.global_to_local_idx), cores)
          ]
...
def test():
  ...
  dataloader = torch.utils.data.DataLoader(
    dataset, shuffle=True, num_workers=0, pin_memory=False, batch_size=4
  )
  print(dataset)
  import time
