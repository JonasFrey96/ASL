...
def plot_from_pkl(main_visu, base_path, task_nr):
  ...
  for m in val_metrices:
    ...
    main_visu.plot_matrix(
      tag=m, data_matrix=data_matrix, higher_is_better=higher_is_better, title=m
    )
...
def validation_acc_plot(main_visu, logger, nr_eval_tasks):
  ...
  y = [
    np.array(df_acc[f"val_acc/dataloader_idx_{idx}"]) for idx in range(nr_eval_tasks)
  ]
...
def plot_from_neptune(main_visu, logger):
  try:
    idxs = logger.experiment.get_numeric_channels_values("task_count/dataloader_idx_0")[
      "x"
    ]
    task_numbers = logger.experiment.get_numeric_channels_values(
    ...
    val_dataloaders = len(
      [
        d
        for d in logger.experiment.get_logs().keys()
        if d.find("task_count/dataloader_idx") != -1
      ]
    )
    ...
    for m in val_metrices:
      ...
      for training_task in range(trained_tasks):
        for val_dataloader in range(val_dataloaders):
          v = logger.experiment.get_numeric_channels_values(
      ...
      if m.find("loss") != -1:
        ...
        higher_is_better = True
      main_visu.plot_matrix(
        tag=m, data_matrix=data_matrix[m], higher_is_better=higher_is_better, title=m
      )
  except:
...
def test():
  ...
  main_visu = MainVisualizer(
    p_visu=os.path.join(os.getenv("HOME"), "tmp"), store=True, num_classes=40
  )
