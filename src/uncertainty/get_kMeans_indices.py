    ...
    while True:
        ...
        for c in range(classes):
            ...
            if ret_val > 0:
                ...
                kmeans = KMeans(n_clusters=ret_val, random_state=0).fit(valid_data.cpu().numpy())
                res = knn(query=torch.from_numpy(kmeans.cluster_centers_), ref = feat[:,c,:].cpu())
                class_candidates.append( res.indices.tolist() ) # contains index of closest cluster center
