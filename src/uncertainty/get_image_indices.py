...
def get_image_indices(
  ...
  if dis_metric == "cos":
    metric = F.cosine_similarity(
      feat.type(torch.float32), expanded_centroids.type(torch.float32), dim=1, eps=1e-6
    )
  elif dis_metric == "pairwise":
    # TODO verify pairwise metric
    metric = F.pairwise_distance(
      feat.type(torch.float32), expanded_centroids.type(torch.float32), eps=1e-6
    )
  else:
...
def test():
  ...
  print("START")
  ret_gloable_indices = get_image_indices(
    data, torch.arange(0, data.shape[0], device=data.device), pick_mode="class_balanced"
  )
  ret_gloable_indices = get_image_indices(
    data, torch.arange(0, data.shape[0], device=data.device), pick_mode="most_hits"
  )
  print(data.device)
