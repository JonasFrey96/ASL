...
def met_low_distance(candidate, buffer, l2=True):
  ...
  """
  if l2:
    inp = candidate[None, :].repeat(buffer.shape[0], 1).type(torch.float32) - buffer[
      :, :
    ].type(torch.float32)
...
def compute_interclass_similarity_score(candidate_features, buffer_features_all):
  ...
  """
  buffer_features_all = torch.cat(
    [candidate_features[None], buffer_features_all], dim=0
  )
...
def interclass_dissimilarity(
  latent_features, label_dist, K_return=50, iterations=1000, early_stopping=0.00001
):
  ...
  for i in range(iterations):
    ...
    new_score = compute_interclass_similarity_score(
      latent_features[replacement_candidate], buffer_features_all[buffer_mask]
    )
    ra = random.random()
...
def test():
  ...
  vis = Visualizer(
    "/home/jonfrey/tmp", logger=None, epoch=0, store=True, num_classes=41
  )

  label_dist = torch.load(
    "/media/scratch1/jonfrey/models/master_thesis/dev/uncertainty_integration3/labels_tensor_0.pt"
  )
  globale_indices = torch.load(
    "/media/scratch1/jonfrey/models/master_thesis/dev/uncertainty_integration3/indices_tensor_0.pt"
  )
  latent_features = torch.load(
