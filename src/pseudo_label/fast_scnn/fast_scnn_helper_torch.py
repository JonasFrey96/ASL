...
if __name__ == '__main__':
    ...
    for j, p in enumerate( paths):
        ...
        h,w,c = i1.shape
        i1 = torch.nn.functional.interpolate(torch.from_numpy(i1.astype(np.float32))[None].permute(0,3,1,2), (320,640), mode='bilinear')[0].permute(1,2,0)
        
        label_probs = fsh.get_label_prob( i1.numpy() )
        label_probs = torch.nn.functional.interpolate(torch.from_numpy( label_probs.astype(np.float32))[None] , (h,w), mode='bilinear')[0].numpy()
