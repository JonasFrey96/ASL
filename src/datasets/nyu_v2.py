...
class NYUv2(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = torch.from_numpy(self.images[index]/255).permute(2,1,0) # C W H 
        label = torch.from_numpy(self.labels[index]).permute(0,1)[None,:,:] # C W H 
    ...
    def _load(self, root, mode):
        ...
        with h5py.File(root, 'r') as f:
            ...
            self.images = np.moveaxis(self.images, 1, 3)
            self.images = np.moveaxis(self.images, 1, 2)
            self.labels = np.moveaxis(self.labels, 1, 2)
            self.labels = np.moveaxis(self.labels, 1, 2)
